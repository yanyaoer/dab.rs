use dab::{player::queue::*, Track, PlayerEvent};
use tokio::sync::mpsc;
...
async fn test_queue_shuffle_functionality() {
    ...
    // Add multiple tracks
    let tracks: Vec<_> = (0..10).map(|i| {
        Track::from_url(&format!("https://example.com/track{}.mp3", i))
    }).collect();
    ...
    // Enable shuffle
    queue.send_command(QueueCommand::SetShuffle(true)).await.unwrap();
    ...
    // Disable shuffle
    queue.send_command(QueueCommand::SetShuffle(false)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_queue_repeat_modes() {
    ...
    queue.send_command(QueueCommand::SetRepeat(RepeatMode::Track)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
    ...
    // Test RepeatMode::All
    queue.send_command(QueueCommand::SetRepeat(RepeatMode::All)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
    ...
    // Test RepeatMode::Off
    queue.send_command(QueueCommand::SetRepeat(RepeatMode::Off)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_queue_remove_track() {
    ...
    // Remove middle track
    queue.send_command(QueueCommand::RemoveTrack(1)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_queue_move_track() {
    ...
    queue.send_command(QueueCommand::MoveTrack(2, 0)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_queue_replace_functionality() {
    ...
    // Add initial tracks
    let initial_tracks: Vec<_> = (0..3).map(|i| {
        Track::from_url(&format!("https://example.com/initial{}.mp3", i))
    }).collect();
    ...
    let new_tracks: Vec<_> = (0..5).map(|i| {
        Track::from_url(&format!("https://example.com/new{}.mp3", i))
    }).collect();
    
    queue.send_command(QueueCommand::ReplaceQueue(new_tracks.clone())).await.unwrap();
    sleep(Duration::from_millis(100)).await;
...
async fn test_queue_jump_to_track() {
    ...
    queue.send_command(QueueCommand::JumpToTrack(3)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_queue_history_tracking() {
    ...
    // Clear history
    queue.send_command(QueueCommand::ClearHistory).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_queue_batch_operations() {
    ...
    queue.send_command(QueueCommand::AddTracks(tracks.clone())).await.unwrap();
    sleep(Duration::from_millis(100)).await;
    ...
    queue.send_command(QueueCommand::RemoveTracks(indices_to_remove)).await.unwrap();
    sleep(Duration::from_millis(100)).await;
