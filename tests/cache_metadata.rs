...
async fn test_cache_metadata_extraction() {
    ...
    let id3_data = vec![
        // ID3v2 header
        0x49, 0x44, 0x33, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3C, // Title frame
        0x54, 0x49, 0x54, 0x32, 0x00, 0x00, 0x00, 0x0B, 0x00, 0x00, 0x54, 0x65, 0x73, 0x74, 0x20,
        0x53, 0x6F, 0x6E, 0x67, // Artist frame
        0x54, 0x50, 0x45, 0x31, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x54, 0x65, 0x73, 0x74, 0x20,
        0x41, 0x72, 0x74, 0x69, 0x73, 0x74, // Album frame
        0x54, 0x41, 0x4C, 0x42, 0x00, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x54, 0x65, 0x73, 0x74, 0x20,
        0x41, 0x6C, 0x62, 0x75, 0x6D, 0x00, // Simple MP3 frame (just a minimal frame)
    ...
    let search_results = cache
        .search_cached_tracks(&SearchQuery::Title("test".to_string()))
        .await
        .unwrap();
    assert_eq!(search_results.len(), 1);
    assert_eq!(search_results[0].track_id, track_id);

    let artist_results = cache
        .search_cached_tracks(&SearchQuery::Artist("artist".to_string()))
        .await
        .unwrap();
    assert_eq!(artist_results.len(), 1);

    let album_results = cache
        .search_cached_tracks(&SearchQuery::Album("album".to_string()))
        .await
        .unwrap();
...
async fn test_cache_unique_id_generation() {
    ...
    let found_track_id = cache
        .get_track_by_unique_id(&entry1.unique_id)
        .await
        .unwrap();
    assert_eq!(found_track_id, Some(track_id1.to_string()));

    assert!(cache
        .has_track_by_unique_id(&entry1.unique_id)
        .await
        .unwrap());
    assert!(!cache
        .has_track_by_unique_id("nonexistent_id")
        .await
        .unwrap());
...
async fn test_cache_metadata_update() {
    ...
    cache
        .update_track_metadata(track_id, new_metadata.clone())
        .await
        .unwrap();
