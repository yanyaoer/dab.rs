...
async fn test_player_commands() {
    ...
    // Test load command
    engine.send_command(PlayerCommand::LoadAndPlayTrack(track.clone())).await.unwrap();
...
async fn test_volume_control() {
    ...
    // Test volume change
    let test_volume = 0.7;
    engine.send_command(PlayerCommand::SetVolume(test_volume)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_seek_functionality() {
    ...
    engine.send_command(PlayerCommand::Seek(seek_position)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
...
async fn test_queue_integration() {
    ...
    engine.send_command(PlayerCommand::AddTrackToQueue(track1.clone())).await.unwrap();
    engine.send_command(PlayerCommand::AddTrackToQueue(track2.clone())).await.unwrap();
    sleep(Duration::from_millis(100)).await;
    ...
    engine.send_command(PlayerCommand::PreviousTrack).await.unwrap();
    sleep(Duration::from_millis(50)).await;
}
...
async fn test_repeat_mode() {
    ...
    engine.send_command(PlayerCommand::SetRepeat(RepeatMode::Off)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
    
    engine.send_command(PlayerCommand::SetRepeat(RepeatMode::Track)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
    
    engine.send_command(PlayerCommand::SetRepeat(RepeatMode::All)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
}
...
async fn test_shuffle_mode() {
    ...
    // Test shuffle toggle
    engine.send_command(PlayerCommand::SetShuffle(false)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
    
    engine.send_command(PlayerCommand::SetShuffle(true)).await.unwrap();
    sleep(Duration::from_millis(50)).await;
}
...
async fn test_player_state_transitions() {
    ...
    // Load a track
    let track = Track::from_url("https://example.com/test.mp3");
    engine.send_command(PlayerCommand::LoadAndPlayTrack(track)).await.unwrap();
    sleep(Duration::from_millis(100)).await;
    
    // State might change to Loading, Playing, or stay as Stopped (depending on implementation)
    let state = engine.get_state().await;
    assert!(matches!(state, PlayerState::Stopped | PlayerState::Loading | PlayerState::Playing | PlayerState::Buffering));
}
...
async fn test_concurrent_commands() {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            engine.send_command(PlayerCommand::AddTrackToQueue(track)).await.unwrap();
        });
        handles.push(handle);
    }
...
async fn test_error_handling() {
    ...
    let result = engine.send_command(PlayerCommand::LoadAndPlayTrack(invalid_track)).await;
