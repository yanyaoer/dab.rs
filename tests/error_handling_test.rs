...
async fn test_error_chain_propagation() {
    ...
    fn outer_operation() -> DabResult<()> {
        inner_operation()
            .map_err(|e| DabError::PlayerError(format!("Player operation failed: {}", e)))
    }
...
async fn test_error_recovery_patterns() {
    fn operation_with_fallback() -> DabResult<String> {
        ...
        let primary_result: Result<String, DabError> = Err(DabError::NetworkError(
...
async fn test_custom_error_types() {
    ...
    fn validate_track_id(id: &str) -> DabResult<()> {
        ...
        {
            return Err(DabError::ApiError(
                "Track ID contains invalid characters".to_string(),
            ));
        }
