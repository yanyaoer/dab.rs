use dab::player::{Queue, Track, QueueCommand, RepeatMode};
...
async fn test_queue_navigation() {
    ...
    sleep(Duration::from_millis(10)).await;
    println!("After previous_track: {:?}", current.as_ref().map(|t| &t.id));
    let current_idx = queue.get_current_index().await;
...
async fn test_queue_commands() {
    ...
    // Test add command
    let _ = queue.send_command(QueueCommand::AddTrack(track1.clone())).await;
    let _ = queue.send_command(QueueCommand::AddTrack(track2.clone())).await;
...
async fn test_queue_repeat_mode() {
    ...
    let _ = queue.send_command(QueueCommand::SetRepeat(RepeatMode::All)).await;
    sleep(Duration::from_millis(50)).await;
