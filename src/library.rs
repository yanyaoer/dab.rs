...
impl Library {
    pub async fn new(cache: Cache) -> DabResult<Self> {
        ...
        info!("Library initialized with {} tracks", library.metadata.tracks.len());
        Ok(library)
    }
    ...
    pub fn get_tracks_by_album(&self, album_title: &str) -> Vec<Track> {
        if let Some(album) = self.metadata.albums.values()
            .find(|a| a.title == album_title) {
            album.tracks.iter()
                .filter_map(|track_id| {
                    self.metadata.tracks.get(track_id).map(|meta| {
                        Track {
                            id: meta.id.clone(),
    ...
    fn update_artist_album_metadata(&mut self, artist_name: &str, album_title: &str, track_id: &str) {
        // Update or create artist
        self.metadata.artists.entry(artist_name.to_string())
            .and_modify(|artist| {
        ...
        self.metadata.albums.entry(album_title.to_string())
            .and_modify(|album| {
