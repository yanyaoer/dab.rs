...
impl Cache {
    ...
    pub async fn store_track_with_url(
        ...
        let unique_id = self.generate_unique_id(track_id, &id3_metadata, original_url).await;
    ...
    async fn extract_id3_metadata(&self, file_path: &Path) -> DabResult<Option<Id3Metadata>> {
        ...
        match id3::Tag::read_from_path(file_path) {
            ...
            Err(e) => {
                debug!("Failed to read ID3 tags from {}: {}", file_path.display(), e);
                Ok(None)
            }
        }
    }
    ...
    async fn generate_unique_id(&self, track_id: &str, id3_metadata: &Option<Id3Metadata>, url: &str) -> String {
        use sha2::{Digest, Sha256};
    ...
    pub async fn has_track_by_unique_id(&self, unique_id: &str) -> DabResult<bool> {
        self.get_track_by_unique_id(unique_id).await.map(|opt| opt.is_some())
    }
    
    /// Search cached tracks by metadata
    pub async fn search_cached_tracks(&self, query: &SearchQuery) -> DabResult<Vec<CachedTrackInfo>> {
        ...
        for (track_id, entry) in &self.metadata.entries {
            ...
            let matches = match query {
                ...
                SearchQuery::General(query) => {
                    ...
                    let title_matches = entry.id3_metadata.as_ref()
                        .and_then(|m| m.title.as_ref())
                        .map(|t| t.to_lowercase().contains(&query_lower))
                        .unwrap_or(false);
                    let artist_matches = entry.id3_metadata.as_ref()
                        .and_then(|m| m.artist.as_ref())
                        .map(|a| a.to_lowercase().contains(&query_lower))
                        .unwrap_or(false);
                    let album_matches = entry.id3_metadata.as_ref()
                        .and_then(|m| m.album.as_ref())
                        .map(|a| a.to_lowercase().contains(&query_lower))
                        .unwrap_or(false);
    ...
    pub async fn get_album_tracks(&self, album_name: &str) -> DabResult<Vec<CachedTrackInfo>> {
        ...
        tracks.sort_by(|a, b| {
            let a_track_num = a.metadata.as_ref().and_then(|m| m.track_number).unwrap_or(0);
            let b_track_num = b.metadata.as_ref().and_then(|m| m.track_number).unwrap_or(0);
        ...
        for entry in self.metadata.entries.values() {
            ...
            if let Some(ref metadata) = entry.id3_metadata {
                if let Some(ref album_name) = metadata.album {
                    let album_info = albums.entry(album_name.clone()).or_insert_with(|| AlbumInfo {
                        name: album_name.clone(),
                        artist: metadata.album_artist.clone().or_else(|| metadata.artist.clone()),
                        year: metadata.year,
    ...
    pub async fn update_track_metadata(&mut self, track_id: &str, new_metadata: Id3Metadata) -> DabResult<()> {
        if let Some(entry) = self.metadata.entries.get_mut(track_id) {
            entry.id3_metadata = Some(new_metadata);
            // Regenerate unique ID with updated metadata
            let unique_id = Self::generate_unique_id_static(track_id, &entry.id3_metadata, &entry.url).await;
            entry.unique_id = unique_id;
            self.save_metadata().await?;
            Ok(())
        } else {
            Err(DabError::Cache(format!("Track {} not found in cache", track_id)))
        }
    }
    
    /// Static version of generate_unique_id for use in methods that need to avoid borrowing conflicts
    async fn generate_unique_id_static(track_id: &str, id3_metadata: &Option<Id3Metadata>, url: &str) -> String {
        use sha2::{Digest, Sha256};
