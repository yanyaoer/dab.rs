...
use super::{PlayerCommand, PlayerEvent, PlayerStatus, PlayerState, Track, Queue};
...
impl PlayerEngine {
    ...
    async fn handle_command(
        ...
        match command {
            PlayerCommand::LoadAndPlay(track_id) => {
                Self::load_and_play_track(&track_id, event_tx, state, current_track, position_ms, volume, queue, loader, audio_sink).await?;
            }
            PlayerCommand::Play => {
                ...
                match current_state {
                    ...
                    PlayerState::Stopped => {
                        if let Some(track) = queue.next_track().await {
                            Self::load_and_play_track(&track.url, event_tx, state, current_track, position_ms, volume, queue, loader, audio_sink).await?;
                        }
                    }
                    _ => {}
                }
    ...
    pub async fn load_and_play(&mut self, track_id: &str) -> DabResult<()> {
        self.send_command(PlayerCommand::LoadAndPlay(track_id.to_string())).await
    }
    ...
    pub async fn add_to_queue(&mut self, url: &str) -> DabResult<()> {
        self.send_command(PlayerCommand::AddToQueue(url.to_string())).await
    }
    
    pub async fn get_status(&mut self) -> DabResult<PlayerStatus> {
        let (tx, rx) = oneshot::channel();
        self.send_command(PlayerCommand::GetStatus(tx)).await?;
        rx.await.map_err(|_| DabError::Player("Failed to get status".to_string()))
    }
