...
impl AudioLoader {
    ...
    pub async fn load_track_seekable(&self, track: &Track) -> DabResult<Box<dyn ReadSeek>> {
        ...
        if let Some(cached_path) = self.cache.get_track_path(&track.id).await? {
    ...
    async fn download_and_cache_track(&self, track: &Track) -> DabResult<Box<dyn std::io::Read + Send + Sync>> {
        ...
        let cache_path = cache_clone.store_track(&track.id, temp_file.path()).await?;
    ...
    pub async fn preload_track(&self, track: &Track) -> DabResult<()> {
        if self.cache.has_track(&track.id).await? {
    ...
    async fn download_and_return_seekable(&self, track: &Track) -> DabResult<Box<dyn ReadSeek>> {
        ...
        // Store in cache for future use (in background)
        let mut cache_clone = self.cache.clone();
        let track_id = track.id.clone();
        let temp_path = temp_file.path().to_path_buf();
        tokio::spawn(async move {
            if let Err(e) = cache_clone.store_track(&track_id, &temp_path).await {
