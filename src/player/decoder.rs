...
use symphonia::core::formats::{FormatOptions, FormatReader, SeekTo, SeekMode};
use symphonia::core::meta::MetadataOptions;
use symphonia::core::codecs::{DecoderOptions, Decoder, CODEC_TYPE_NULL};
...
impl AudioDecoder {
    pub fn new(_source: Box<dyn Read + Send + Sync>) -> DabResult<Self> {
        ...
        Err(DabError::Decode("Audio decoder requires seekable source".to_string()))
    }
    ...
    pub fn seek(&mut self, position_ms: u32) -> DabResult<()> {
        let time_base = self.format.default_track()
            .ok_or_else(|| DabError::Decode("No default track".to_string()))?
            .codec_params
            .time_base;
            
        if let Some(time_base) = time_base {
            let timestamp = (position_ms as u64 * time_base.numer as u64) / 
                           (1000 * time_base.denom as u64);
